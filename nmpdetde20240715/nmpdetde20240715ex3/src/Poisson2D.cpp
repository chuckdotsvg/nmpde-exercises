...
void Poisson2D::setup(double left, double right) {
  // Create the mesh.
  {
    std::cout << "Initializing the mesh" << std::endl;

    GridGenerator::subdivided_hyper_cube(mesh, N_el, left, right, true);
    ...
    std::string mesh_file_name =
        "mesh-" + std::to_string(subdomain_id) + ".vtk";
    GridOut grid_out;
    ...
    for (unsigned int q = 0; q < n_q; ++q) {
      for (unsigned int i = 0; i < dofs_per_cell; ++i) {
        for (unsigned int j = 0; j < dofs_per_cell; ++j) {
          cell_matrix(i, j) += eps * fe_values.shape_grad(i, q) *
                               fe_values.shape_grad(j, q) * fe_values.JxW(q);

          cell_matrix(i, j) += b * fe_values.shape_grad(i, q)[0] *
                               fe_values.shape_value(j, q) * fe_values.JxW(q);

          cell_matrix(i, j) += c * fe_values.shape_value(i, q) *
                               fe_values.shape_value(j, q) * fe_values.JxW(q);
        }
...
void Poisson2D::output(const unsigned int &iter) const {
...
void Poisson2D::apply_interface_dirichlet(const Poisson2D &other) {
...
void Poisson2D::apply_interface_neumann(Poisson2D &other) {
  const auto interface_map = compute_interface_map(other);
...
Poisson2D::compute_interface_map(const Poisson2D &other) const {
  ...
  if (subdomain_id == 0) {
    ...
    other_interface_dofs = DoFTools::extract_boundary_dofs(
        other.dof_handler, ComponentMask(), {0});
  } else {
    current_interface_dofs =
        DoFTools::extract_boundary_dofs(dof_handler, ComponentMask(), {0});
    other_interface_dofs = DoFTools::extract_boundary_dofs(
        other.dof_handler, ComponentMask(), {1});
  }
