    ...
    for (unsigned int q = 0; q < n_q; ++q) {
      for (unsigned int i = 0; i < dofs_per_cell; ++i) {
        for (unsigned int j = 0; j < dofs_per_cell; ++j) {
          // time derivative
          cell_matrix(i, j) +=
              (1.0 / delta_t) * fe_values[velocity].value(i, q) *
              fe_values[velocity].value(j, q) * fe_values.JxW(q);
          // a(u, v) term in the weak formulation.
          cell_matrix(i, j) +=
              theta * mu *
          ...
          // b(p, v) term in the weak formulation. (momentum equation)
          cell_matrix(i, j) -= fe_values[velocity].divergence(i, q) *
                               fe_values[pressure].value(j, q) *
                               fe_values.JxW(q);

          // b(q, u) term in the weak formulation. (continuity equation)
        ...
        // time derivative
        cell_rhs(i) += (1 - theta) * (1.0 / delta_t) *
                       fe_values[velocity].value(i, q) *
                       solution_old_values[q] * fe_values.JxW(q);

        // a(u_old, v) term in the weak formulation.
        cell_rhs(i) += (1 - theta) * mu *
                       scalar_product(fe_values[velocity].gradient(i, q),
          ...
          for (unsigned int f = 0; f < cell->n_faces(); ++f) {
            if (cell->face(f)->at_boundary() &&
                cell->face(f)->boundary_id() <
                    2) // Neumann BCs are on boundary ids 0 and 1.
            {
              fe_face_values.reinit(cell, f);

              // neumann bc: since no time dependency, at different times the
              // value is the same, so we can use the old solution to compute
              // the value at the current time step.
              cell_rhs(i) += -(cell->face(f)->boundary_id() + 1.0) *
...
void NavierStokes::output() {
  ...
  data_out.write_vtu_with_pvtu_record("./", output_file_name, timestep_number,
                                      MPI_COMM_WORLD);
...
void NavierStokes::run() {
  ...
  // Time-stepping loop.
  while (current_time < T - 0.5 * delta_t) {
