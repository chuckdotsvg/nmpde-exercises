...
Poisson2D::assemble()
{
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  cell_matrix(i, j) += gamma * fe_values.shape_value(i, q) *
                                       fe_values.shape_value(j, q) *
                                       fe_values.JxW(q);
                }

              cell_rhs(i) += fe_values.shape_value(i, q) *
                             fe_values.JxW(q);  // Right-hand side = 1.0
            }
        }
  ...
  // Boundary conditions.
  {
    ...
    boundary_functions[2] = &function_bc;
    boundary_functions[3] = &function_bc;
...
Poisson2D::output(const unsigned int &iter) const
{
  ...
  // output inside vtks folder

  const std::string folder = "vtks";

  const std::string output_file_name = "./vtks/output-" +
