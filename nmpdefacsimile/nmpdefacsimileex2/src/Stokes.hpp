...
public:
  ...
  class InletVelocity1 : public Function<dim> {
  public:
    InletVelocity1() : Function<dim>(dim + 1) {}

    virtual void vector_value(const Point<dim> &p,
                              Vector<double> &values) const override {
      values[0] = 27. / 4. * p[1] * (1.0 - p[1]) * (1.0 - p[1]);

      // to avoid errors due to incompatible number of components, we set the
      // remaining components to zero (besides the actual velocity component x2,
      // the other velocity component and the pressure).
    ...
    virtual double value(const Point<dim> &p,
      ...
      if (component == 0)
        return 27. / 4. * p[1] * (1.0 - p[1]) * (1.0 - p[1]);
      else
        return 0.0;
    }
  };
  ...
  public:
    ...
    void vmult(TrilinosWrappers::MPI::BlockVector &dst,
      ...
      solver_cg_velocity.solve(*velocity_stiffness, dst.block(0), src.block(0),
  ...
  public:
    ...
    void vmult(TrilinosWrappers::MPI::BlockVector &dst,
      ...
      SolverCG<TrilinosWrappers::MPI::Vector> solver_cg_pressure(
          solver_control_pressure);
      solver_cg_pressure.solve(*pressure_mass, dst.block(1), tmp,
                               preconditioner_pressure);
    }
